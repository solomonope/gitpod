...
Create a Gitpod configuration for this project.
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if !interactive {
			dockerFileDeclaration := ``
			if dockerFile {
				dockerFileDeclaration = `image:
  file: .gitpod.Dockerfile
  
  `
			}

			d = []byte(dockerFileDeclaration +
...
`)
		...
		if v, ok := cfg.Image.(gitpodlib.GitpodImage); ok {
			if _, err := os.Stat(v.File); os.IsNotExist(err) {
				if err := os.WriteFile(v.File, []byte(`FROM gitpod/workspace-base
...
func init() {
	...
	initCmd.Flags().BoolVarP(&dockerFile, "dockerFile", "d", false, "create a .gitpod.Dockerfile (non interactive mode)")
}
